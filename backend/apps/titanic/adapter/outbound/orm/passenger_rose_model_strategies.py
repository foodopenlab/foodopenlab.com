...
def _as_int_list(predictions: Any) -> list[int]:
    return [int(x) for x in np.asarray(predictions).ravel().tolist()]


def _positive_class_proba(model: Any, X: Any) -> list[float]:
...
class XGBoostStrategy(SurvivalModelStrategy):
    ...
    def predict(self, X) -> list[int]:
        return _as_int_list(self._model.predict(X))

    def predict_proba(self, X) -> list[float]:
        return _positive_class_proba(self._model, X)
...
class LogisticRegressionStrategy(SurvivalModelStrategy):
    ...
    def predict(self, X) -> list[int]:
        return _as_int_list(self._model.predict(self._scaler.transform(X)))

    def predict_proba(self, X) -> list[float]:
        return _positive_class_proba(self._model, self._scaler.transform(X))
