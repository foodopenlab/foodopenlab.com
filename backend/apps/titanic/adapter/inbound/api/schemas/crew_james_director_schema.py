...
class FileUploadSchema(BaseModel):
    ...
    embarked: Optional[str] = Field(None, description="탑승 항구")
...
class TitanicRecordSchema(BaseModel):
    ...
    age: Optional[str] = Field(None, description="나이 (소수 허용)")
    sib_sb: Optional[str] = Field(None, description="동승한 형제자매/배우자 수")
    parch: Optional[str] = Field(None, description="동승한 부모/자녀 수")
    ticket: Optional[str] = Field(None, description="티켓 번호")
    fare: Optional[str] = Field(None, description="탑승 요금 (소수 허용)")
    cabin: Optional[str] = Field(None, description="객실 번호")
...
class UploadResultSchema(BaseModel):
    ...
    count: int = Field(..., description="저장된 레코드 수 (프론트 호환)")
