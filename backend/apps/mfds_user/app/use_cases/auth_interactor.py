...
class AuthInteractor(AuthUseCase):
    ...
    async def signup(self, command: SignupCommand) -> UserSessionDTO:
        ...
        if not whitelist_entry:
            raise ValueError("가입 승인 대상 이메일이 아닙니다. 관리자에게 문의하세요.")
        ...
        # 3. 사용자 및 역할 저장 (expert_users 테이블)
        ...
        user = await self._auth_repository.save_user(
            ...
            role="expert"  # v9의 기본 등록 역할은 expert
        )

        # 4. 세션 발급
        access_token, expires_at = generate_access_token(user.id, user.email, "expert")
    ...
    async def refresh_token(self, refresh_token: str) -> UserSessionDTO:
        ...
        # 새 세션 발급
        access_token, expires_at = generate_access_token(session.user_id, session.email, session.role)
