...
// (기존 백엔드 /regulation-chat 직접 호출 및 프론트 법령 RAG fallback 제거 —
...
export async function POST(req: Request) {
  try {
    ...
    const sessionKey = (
      body.session_key?.trim() || `reg-${crypto.randomUUID().replace(/-/g, "").slice(0, 12)}`
    ).slice(0, 64)
    ...
    console.error("[api/regulation-chat→gateway]", e)
